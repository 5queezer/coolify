@php
    $isHibernated = str($application->status)->startsWith('hibernated');
    $isStopped = str($application->status)->startsWith('exited') || $isHibernated;
@endphp
        ...
        <nav
            ...
            <a class="shrink-0 {{ request()->routeIs('project.application.logs') ? 'dark:text-white' : '' }}"
                ...
                <div class="flex items-center gap-1">
                    Logs
                    @if ($application->restart_count > 0 && !$isStopped)
                    ...
                    <x-dropdown>
                        ...
                        @if (!$isStopped)
                            ...
                            @if ($application->isSablierEnabled())
                                <x-modal-confirmation title="Confirm Application Hibernation?" buttonTitle="Hibernate"
                                    submitAction="hibernate" :actions="[
                                        'This application will be stopped but its container will be kept.',
                                        'Sablier can wake it automatically on the next matching request.',
                                    ]" :confirmWithText="false" :confirmWithPassword="false"
                                    step1ButtonText="Continue" step2ButtonText="Confirm">
                                    <x-slot:trigger>
                                        <div class="dropdown-item dropdown-item-touch text-warning">
                                            Hibernate
                                        </div>
                                    </x-slot:trigger>
                            ...
                            <x-modal-confirmation title="Confirm Application Stopping?" buttonTitle="Stop"
                                submitAction="stop" :checkboxes="$checkboxes" :actions="[
                                    'This application will be stopped and removed.',
                                ...
                                <x-modal-confirmation title="Confirm Application Hibernation?" buttonTitle="Hibernate"
                                    ...
                                    <x-slot:button-title>
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-warning" viewBox="0 0 24 24"
                                            ...
                                            <path d="M12 3a6 6 0 0 0 -6 6c0 7 6 12 6 12s6 -5 6 -12a6 6 0 0 0 -6 -6" />
                                            <path d="M12 7v4" />
                                            <path d="M12 15h.01" />
                                        </svg>
                                        Hibernate
